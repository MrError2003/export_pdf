...
$sql = "SELECT id, number_id, first_name, second_name, first_last, second_last FROM arrendatarios";
$resultado = $conn->query($sql);

// Consulta para traer propiedades de la base de datos
...
// Consulta para obtener la propiedad correspondiente al arrendatario
$sqlPropiedad = "SELECT p.* 
                 FROM propiedades_inmuebles p
                 INNER JOIN arrendatarios a ON p.id = a.vivienda_id 
                 WHERE a.number_id = '" . $number_id . "'";
$resultadoPropiedad = $conn->query($sqlPropiedad);

if ($resultadoPropiedad->num_rows > 0) {
    $propiedad = $resultadoPropiedad->fetch_assoc();
    $direccion = $propiedad['direccion'];
    $ciudad = $propiedad['ciudad'];
    $barrio = $propiedad['barrio']; 
    $valor_canon = $propiedad['valor_canon'];
} else {
    $direccion = "No se encontró dirección";
    $valor_canon = 0;
}

// Consulta para obtener el primer deudor del arrendatario
$sqlDeudor = "SELECT type_id, number_id, first_name, second_name, first_last, second_last 
              FROM deudores 
              WHERE arrendatario_number_id = '" . $number_id . "'
              LIMIT 1";
$resultadoDeudor = $conn->query($sqlDeudor);

if ($resultadoDeudor->num_rows > 0) {
    $rowDeudor = $resultadoDeudor->fetch_assoc();
    $rowDeudor['nombre_completo'] = $rowDeudor['first_name'] . ' ' . 
                                   ($rowDeudor['second_name'] ? $rowDeudor['second_name'] . ' ' : '') . 
                                   $rowDeudor['first_last'] . ' ' .
                                   ($rowDeudor['second_last'] ? $rowDeudor['second_last'] : '');
    // No necesitamos asignar number_id ya que viene en $rowDeudor
} else {
    $rowDeudor = array(
        'nombre_completo' => '',
        'numero_id' => '',
        'number_id' => '' // Añadido para consistencia
    );
}
...
<body>
    ...
    <p><strong>NOMBRE: '. $rowDeudor['nombre_completo'] .' </strong></p>

    <p><strong>CC: '. $rowDeudor['number_id'] .'</strong></p>
    ...
    <table style="width: 100%; border-collapse: collapse; border: 1px solid black;">
        <tr>
            <td style="border-bottom: 1px solid black; padding: 8px;">Nombre:</td>
        </tr>
        <tr>
            <td style="border-bottom: 1px solid black; padding: 8px;">Identificación:</td>
        </tr>
        <tr>
            <td style="border-bottom: 1px solid black; padding: 8px;">Dirección Residencia:</td>
        </tr>
        <tr>
            <td style="border-bottom: 1px solid black; padding: 8px;">Dirección laboral:</td>
        </tr>
        <tr>
            <td style="border-bottom: 1px solid black; padding: 8px;">Correo electrónico:</td>
        </tr>
        <tr>
            <td style="border-bottom: 1px solid black; padding: 8px;">Tels. fijos y móviles:</td>
        </tr>
        <tr>
